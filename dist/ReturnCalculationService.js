...
class ReturnCalculationService {
    constructor(input, purchasePrice, mortgageService, expenseService) {
        this.input = input;
         ...
         */
        this.calculatePrincipalReduction = (remainingLoanBalance = this.mortgageService.getLoanAmount()) => {
            const annualInterestPayment = remainingLoanBalance * (this.input.annualMortgageInterestRate / 100);
            return (this.mortgageService.getMonthlyMortgagePayment() * 12 -
        ...
        this.getInitialInvestment = () => {
            return (this.expenseService.getClosingCosts() +
         ...
         */
        this.getReturns = () => {
            const years = RealEstateDealAnalyser_1.FINANCIAL_CONSTANTS.INVESTMENT_YEAR_TIME_HORIZON;
            const initialInvestment = this.getInitialInvestment();
            const returns = { yields: [], amounts: [] };
            let metrics = this.getInitialMetrics();
            for (let year = 1; year <= years; year++) {
                metrics = this.calculateYearlyMetrics(year, metrics);
                const annualReturn = metrics.cashFlow + metrics.principalPaid + metrics.appreciation;
                returns.amounts.push(annualReturn);
                returns.yields.push(annualReturn / initialInvestment);
            }
            return Object.assign(Object.assign({}, returns), { avgYield: ReturnCalculationService.getArrAvg(returns.yields), avgAmount: ReturnCalculationService.getArrAvg(returns.amounts) });
        };
        /**
        ...
        this.getPrincipalReductions = () => {
            ...
            return {
                principalReductions,
                avgPrincipalReduction: ReturnCalculationService.getArrAvg(principalReductions),
     ...
     */
    calculateAppreciation(appreciationYear = 1) {
        if (appreciationYear < 1)
            return 0;
        if (this.input.unitCount > 1) {
            let newRent = this.input.monthlyRent;
            const rentIncreaseRate = 1 + RealEstateDealAnalyser_1.FINANCIAL_CONSTANTS.APPRECIATION_RATES.RENT;
            for (let year = 1; year <= appreciationYear; year++) {
                newRent *= rentIncreaseRate;
            }
            const prevRent = newRent / rentIncreaseRate;
            const prevValue = (prevRent * this.purchasePrice) / this.input.monthlyRent;
        ...
        for (let year = 1; year <= appreciationYear; year++) {
            prevValue = currValue;
            currValue *= 1 + RealEstateDealAnalyser_1.FINANCIAL_CONSTANTS.APPRECIATION_RATES.SFH;
        }
    ...
    getIRR() {
        try {
            const irr = ReturnCalculationService.calculateIRR(this.getCashflows().cashflows);
 ...
 */
ReturnCalculationService.getArrAvg = (arr) => {
 ...
 * @returns The Internal Rate of Return (IRR) as a decimal value.
 */
ReturnCalculationService.calculateIRR = (cashflows) => {
    ...
    for (let i = 0; i < maxIterations; i++) {
        ...
        const npv = cashflows.reduce((acc, cashflow, index) => acc + cashflow / (1 + mid) ** index, 0);
...
exports.default = ReturnCalculationService;
