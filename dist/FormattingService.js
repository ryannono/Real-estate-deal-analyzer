...
class FormattingService {
    constructor(config = {}) {
        ...
        this.titleCaseMap = new Map();
        this.acronyms = new Set(['ROI', 'CoC', 'PMI', 'HOA', 'CapEx']);
        /**
        ...
        this.initializeFormatters = () => {
            try {
                ...
                this.formatters.set('percentage', new Intl.NumberFormat(this.config.locale, {
                    style: 'percent',
                    minimumFractionDigits: this.config.percentageDigits,
                    maximumFractionDigits: this.config.percentageDigits,
                }));
                // Number formatter
                this.formatters.set('number', new Intl.NumberFormat(this.config.locale, {
                ...
                this.formatters.set('compact', new Intl.NumberFormat(this.config.locale, {
                    notation: 'compact',
                    compactDisplay: 'short',
                }));
            }
            catch (error) {
                throw new FormattingError(`Failed to initialize formatters: ${error.message}`);
            }
        };
        /**
        ...
        this.formatTitle = (str) => {
            ...
            if (cached)
                return cached;
            if (!this.config.formatTitleCase)
                return str;
            const formatted = str
                .replace(/([A-Z])/g, ' $1')
         ...
         */
        this.formatValue = (value, formatter) => {
            if (typeof value === 'string')
                return value;
            const numberFormatter = this.formatters.get(formatter !== null && formatter !== void 0 ? formatter : 'text');
            if (numberFormatter) {
         ...
         */
        this.generateSection = ({ title, description, data, defaultFormatter, collapsed, }) => {
            const lines = Object.entries(data).map(([key, fieldData]) => {
                ...
                if (typeof fieldData === 'object' && 'value' in fieldData) {
                    ...
                    formatter = (_a = fieldData.formatter) !== null && _a !== void 0 ? _a : defaultFormatter;
                    note = fieldData.note;
                ...
                let line = `${this.formatTitle(key)}: ${this.formatValue(value, formatter)}`;
                if (note) {
                    line += ` _(${note})_`;
                }
                if (highlight) {
            ...
            if (collapsed) {
                output.push('<details>', '<summary>Show details</summary>', '', ...lines, '', '</details>');
            }
            else {
                output.push(...lines);
            }
        ...
        this.generateMarkdownReport = (reportData) => {
            try {
                ...
                throw new FormattingError(`Failed to generate report: ${error.message}`);
            }
        };
        /**
        ...
        this.config = {
            locale: (_a = config.locale) !== null && _a !== void 0 ? _a : 'en-CA',
            currency: (_b = config.currency) !== null && _b !== void 0 ? _b : 'CAD',
            currencySignDisplay: (_c = config.currencySignDisplay) !== null && _c !== void 0 ? _c : 'auto',
            percentageDigits: (_d = config.percentageDigits) !== null && _d !== void 0 ? _d : 2,
            numbersDigits: (_e = config.numbersDigits) !== null && _e !== void 0 ? _e : 2,
            suppressTrailingZeros: (_f = config.suppressTrailingZeros) !== null && _f !== void 0 ? _f : true,
            sectionTitleLevel: (_g = config.sectionTitleLevel) !== null && _g !== void 0 ? _g : 2,
            formatTitleCase: (_h = config.formatTitleCase) !== null && _h !== void 0 ? _h : true,
        };
        this.initializeFormatters();
    }
}
